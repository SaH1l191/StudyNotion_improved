...
import {
  deleteCourse,
  fetchInstructorCourses, getFullDetailsOfCourse
...
export default function CoursesTable({ courses, setCourses }) {
  ...
  const getDuration = async (courseId) => {
    setLoading(true);

    const res = await getFullDetailsOfCourse(courseId, token);
    console.log("Response consisting of courseDetails + totalDuration: ", res);

    if (res && res.totalDuration) {

      setDuration((prevDuration) => ({
        ...prevDuration,
        [courseId]: res.totalDuration,
      }));
    }
  ...
  useEffect(() => {
    // Fetch duration for each course initially (optional)
    courses?.forEach((course) => getDuration(course._id));
    console.log("use effect above ")
  }, [courses]);
            ...
            courses?.map((course) => (
              ...
              >
                ...
                <Td className="text-sm font-medium text-richblack-100">
                  {duration[course._id] ? `${duration[course._id]} ` : "Loading..."}
                </Td>
                <Td className="text-sm font-medium text-richblack-100">
                  ₹{course.price}
                </Td>
